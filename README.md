...
Расширение Chrome, которое сохраняет кадры, ролики и звук из Instagram,
TikTok и YouTube одним нажатием. Личный инструмент для сбора референсов.
...
или `/@автор/photo/…`, в YouTube — `/watch?v=…` или `/shorts/…`.
...
без разбора ролика. Ролик берётся без водяного знака.

**YouTube устроен иначе.** Он не отдаёт готовых файлов: видео и звук идут
раздельными потоками, а адреса закрыты подписью. Расширение ничего не
расшифровывает — оно берёт те адреса, которые плеер уже подписал сам, чтобы
проиграть ролик, и собирает два потока в один файл.

Из этого следует одна особенность: расширение видит только то, что плеер
успел запросить. Если кнопка говорит «Потоки закрыты подписью», включи
воспроизведение на пару секунд и нажми снова. Качество берётся то, которое
сейчас выставлено в плеере.

Крупные файлы уходят в корень Загрузок, а не в папку: собранное видео
передаётся Chrome целиком, и для больших роликов подпапку задать нельзя.
У длинных роликов собрать 1080p со звуком в памяти не выйдет: это легко под
гигабайт. В таком случае расширение само возьмёт прогрессивный формат — он
ниже качеством, зато готов одним файлом — и скажет в тосте, сколько весила бы
сборка. Совсем большие ролики сохранять откажется и назовёт размер.

Если у поста есть звук, слева появляется кнопка с нотой. Она сохраняет только
звуковую дорожку: ролик скачивается в память, звук вынимается из него без
перекодирования и ложится рядом как `m4a`. Качество остаётся исходным, и
длительность совпадает с роликом.

У фотопоста с прикреплённой музыкой ролика нет, поэтому берётся тот файл
дорожки, который отдаёт сам Instagram.
...
| `src/lib/mp4mux.js` | Сборка раздельных потоков видео и звука в один mp4. Понимает фрагментированный контейнер. Покрыт тестами |
...
скачивание закреплённых альбомов целиком, плейлисты, субтитры, автообновление,
публикация в Chrome Web Store.

Подписи и защита не обходятся. На YouTube используются только те адреса,
которые страница произвела сама для себя.
